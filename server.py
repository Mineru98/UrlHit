...
def _search():
    ...
    if count[0][0] == 0:
        ...
        cs.close()
        conn.close()
        data = {"Code": "200", "url": 'https://blog-mineru.run.goorm.io/url/'+compat_url, "size": 1, "tag": {"1":"Default"}}
        # data = {"Code": "200", "url": 'http://www.urlhit.shop/url/'+compat_url, "size": 1, "tag": {"1":"Default"}}
        return data
    elif count[0][0] > 0:
...
def _addtag():
    ...
    cs.close()
    conn.close()
    data = {"Code": "201", "url": 'https://blog-mineru.run.goorm.io/url/'+compat_url, "tag": request.get_json()['tag']}
    # data = {"Code": "201", "url": 'http://www.urlhit.shop/url/'+compat_url, "tag": request.get_json()['tag']}
    return data
'''
type = 유입 플랫폼
'''
